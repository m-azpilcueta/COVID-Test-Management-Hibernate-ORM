...
public class PruebaDaoJPA implements PruebaDao {
	...
	public void elimina(Prueba prueba) {
		try {
			...
			if (LocalDateTime.now().isBefore(pruebaTmp.getFecha())) {
				...
				throw new RuntimeException("¡La prueba ya ha tenido lugar!");
	...
	public Prueba recuperaPorCod(String cod) {
		...
		try {
			...
			pruebas = em.createNamedQuery("Prueba.findByCodigo", Prueba.class).setParameter("codPrueba", cod)
					.getResultList();
	...
	@Override
	public Map<String, Integer> recuperaPositivosPorLocalidad() {
		...
		try {
			...
			result = em.createNamedQuery("Prueba.numeroPositivosLocalidad", Tuple.class).getResultStream()
					.collect(Collectors.toMap(tuple -> tuple.get("localidad").toString(),
							tuple -> ((Number) tuple.get("positivos")).intValue()));
	...
	@Override
	public Map<String, Integer> recuperaLocalidadMasTest() {
		Map<String, Integer> result = null;
		...
		try {
			...
			result = em.createNamedQuery("Prueba.localidadMasTests", Tuple.class).getResultStream()
					.collect(Collectors.toMap(tuple -> tuple.get("localidad").toString(),
							tuple -> ((Number) tuple.get("tests")).intValue()));
